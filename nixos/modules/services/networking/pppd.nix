...
in
{
  ...
  config = let
    # pppd likes to write directly into /var/run. This is rude on a
    # modern system, so this wrapper transparently moves those files
    # into /run/pppd.
    pppd-wrapped = pkgs.stdenv.mkDerivation {
      name = "pppd-wrapped";
      phases = [ "installPhase" ];
      buildInputs = [ pkgs.makeWrapper ];
      installPhase = ''
        mkdir -p $out/sbin
        makeWrapper ${getBin cfg.package}/sbin/pppd $out/sbin/pppd \
    ...
    mkSystemd = peerCfg: {
      "pppd-${peerCfg.name}" = {
        ...
        serviceConfig = {
          ExecStart = "${getBin pppd-wrapped}/sbin/pppd call ${peerCfg.name} nodetach nolog";
          Restart = "always";
          RestartSec = 5;
